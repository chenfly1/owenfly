...
const ExportExcel = (sheets: exportProps[] | string, fileName: string = 'Excel文件') => {
  ...
  const getColWidth = (headers: any[], dataArr: any[]) => {
    const allWch = [headers].concat(dataArr).map((item) =>
      item.map((val) => {
  ...
  const sheetsArr = sheets.map((sheet) => {
    const _headers = sheet.headers
      .map((item, i) =>
        Object.assign({}, { key: item.key, title: item.title, position: getPostition(i) + 1 }),
      )
      .reduce(
        (prev, next) =>
    ...
    const dataArr = sheet.data.map((item, i) =>
      sheet.headers.map((head, j) => {
        let content = '';
        if (head.render) {
    ...
    return Object.assign({ sheetName: sheet.sheetName }, output, {
      '!ref': ref,
  ...
  const wbSheets = sheetsArr.reduce(
    (prev, next) => Object.assign({}, prev, { [next.sheetName]: next }),
    {},
  );
