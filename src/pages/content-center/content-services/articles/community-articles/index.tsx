...
import { onlineArticle, queryActivityPage, queryArticlePage } from '@/services/content';
...
export default () => {
    ...
    if (res.code === 'SUCCESS') {
      ...
      if (!row.status) {
        ...
        Modal.confirm({
          ...
          cancelText: '取消',
          onOk: async () => {
            history.push('/content-center/marketing-management/owner-marketing');
          },
          onCancel() {
            console.log('Cancel');
          },
  ...
  useEffect(() => {
    ...
    // 安防跳转过来
    if (query.id && !query.formType) {
      setEditData({ id: query.id });
      setDetailsModalVisit(true);
    }
    // 活动跳转过来
    if (query.formType === 'activity') {
      setEditData({ id: query.id, formType: 'activity', activityId: query.activityId });
  ...
  const columns: ProColumns<ArticleContentPageType>[] = [
    ...
    },
    {
      ...
      hideInTable: true,
      request: async () => {
        const res = await queryActivityPage({
          pageNo: 1,
          pageSize: 100000,
        });
        console.log(res);
