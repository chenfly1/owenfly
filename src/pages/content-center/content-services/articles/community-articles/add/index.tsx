...
import TransferTalk from '@/pages/content-center/components/transferTalk';
...
const AddModelForm: React.FC<IProps> = (props) => {
  ...
  useEffect(() => {
    ...
    setTitle('填写文章信息');
    setObjectId('');
    if (data?.formType === 'activity' && !data?.id) {
      getActivityDetails(data?.activityId).then((res) => {
        const actData = res.data;
        if (res.code === 'SUCCESS') {
          formRef?.current?.setFieldsValue({
            projectBids: [actData.projectBid],
            projectNames: actData.projectName,
            activityTitle: actData.title,
          });
          setActivityId(data?.activityId);
          setProjectList([actData.projectBid]);
        }
      });
    }
    if (data?.id) {
  ...
  return (
    ...
    >
      ...
      {data?.id || isOneProject || activityId ? (
      ...
      <ProFormSelect
        ...
        request={async () => {
          ...
          if (res && (res as any).code === 'SUCCESS') {
            if (data?.formType === 'activity' && !data?.id) {
              const info = (res as any).data.filter((i: any) => i.buildIn === 'ACTIVITY');
              formRef?.current?.setFieldsValue({
                authorId: info && info.length ? info[0].id : null,
              });
            }
            return (res as any).data.map((i: any) => ({
              value: i.id,
              label: i.status ? i.name : `${i.name}（已下线）`,
            }));
          } else {
            return [];
          }
      ...
      />
      <ProFormText colon={false} name="activityTitle" readonly label="关联活动" />
      <ProFormRadio.Group
        name="isHomeLetter"
        colon={true}
        label="是否为项目家书"
      ...
      <ProFormItem shouldUpdate>
        {(form) => {
          console.log(form);
          const projectBids = form?.getFieldValue('projectBids');
          const topicIds = form?.getFieldValue('topicIds');
          return (
            <ProFormItem
              label="关联话题"
              name="topicIds"
