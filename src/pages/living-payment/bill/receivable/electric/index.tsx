...
import { electricColumns } from '../columns';
...
export default ({ month }: IProps) => {
  ...
  const queryList = async (params: any) => {
    ...
    const res = await billListElectric(params);
    return {
  ...
  const columns: ProColumns<BillListElectricType>[] = [
    {
      title: '项目名称',
  ...
  const exportClick = async () => {
    ...
    Method.exportExcel(
      `/content/mng/bill_list/electric`,
      `电费账单_${dayjs().format('YYYY-MM-DD HH:mm:ss')}`,
      {
        ...params,
        excel: 'export',
      },
