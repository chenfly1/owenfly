...
import { parkYardListByPage, transRecordsLt } from '@/services/park';
...
export default () => {
  ...
  const formColumns: ProColumns<TransRecordsltType>[] = [
    {
  ...
  const temporaryColumns1: ProColumns[] = temporaryColumns.map((item) => {
    if (
      ['totalAmount', 'paidAmount', 'discountAmount', 'refundAmount'].includes(
            ...
            actions={[
              {
                ...
                onClick() {
                  history.push({
                    ...
                    query: { id: row.id, type: 'lt', status: row.refundStatus },
                  });
                },
              },
              {
  ...
  const columns1: ProColumns[] = [...formColumns1, ...temporaryColumns1, ...columns2];

  const queryList = async (params: any) => {
    params.pageNo = params.current;
    const res = await transRecordsLt(params);
    return {
  ...
  const onExportClick = async () => {
    ...
    params.excel = 'export';
    exportExcel('/parking/mng/trans_records/lt', '临停交易记录', params);
  };
