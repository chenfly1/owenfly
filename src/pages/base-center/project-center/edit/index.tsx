...
import { modifyProject, getProjectDetails, getProjectAllList } from '@/services/mda';
...
const ProjectEdit: FC<Record<string, any>> = () => {
  ...
  const onFinish = async (values: Record<string, any>) => {
    try {
      const res = await modifyProject((query as any).id, {
        ...values,
        id: Number((query as any).id),
        ...
        addStageNames: stageList.filter((i) => !i.id).map((i) => i.name),
      });
      if (res.code === 'SUCCESS') {
        // 刷新列表
  ...
  const getDetails = async () => {
    setLoading(true);
    const res = await getProjectDetails((query as any).id);
    setLoading(false);
    const stageLists = res.data.projectStageVOList
      ? res.data.projectStageVOList.map((item) => {
          item.open = false;
          return item;
        })
      : [];
    setStageList(stageLists as any);
    setObjectId(res.data.projectDrawingUrl as any);
    const urlRes = await generateGetUrl({
      bussinessId: publicMaterialLib.id,
      urlList: [
        {
          objectId: res.data.projectDrawingUrl as string,
        },
      ],
    });
    res.data.projectDrawingUrl = [
      {
    ...
    return {
      ...
      area: res.data.region
        ? [res.data.province, res.data.city, res.data.region]
        : [res.data.province, res.data.city],
      // orgBid: JSON.parse(res.data.orgBid),
    };
  };
  ...
  return (
    ...
    >
      <ProForm
        ...
        onFinish={onFinish}
      >
        <Card title="基础信息" className={styles.card} bordered={false} loading={loading}>
          <Row gutter={16}>
            ...
            {/* <Col lg={24} md={24} sm={24}>
              <ProFormText label={fieldLabels.stage} name="name" readonly>
