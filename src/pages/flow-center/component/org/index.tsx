...
export const FlowOrg: React.FC<IProps & Record<string, any>> = ({ value, onChange }) => {
  ...
  const toggleCheckbox = (ids: string[], flag: boolean) => {
    ...
    let checkAll = false;
    if (myLeftList.some((item) => item.type === NodeTypeEnum.org && item.checked)) {
      indeterminate = true;
    }
    if (myLeftList.every((item) => item.type === NodeTypeEnum.org && item.checked)) {
      indeterminate = false;
      checkAll = true;
    }
  ...
  const changeAll = () => {
    let flag = true;
    if (leftList.filter((item) => item.type === NodeTypeEnum.org).every((item) => item.checked)) {
      flag = false;
    }
    const ids = leftList.filter((item) => item.type === NodeTypeEnum.org).map((item) => item.code);
    ...
    try {
      ...
      const fooSort = (list: StaffType[]) => {
        list.sort((a: StaffType, b: StaffType) => {
          const asort = a.type === NodeTypeEnum.org ? 0 : 1;
          const bsort = b.type === NodeTypeEnum.org ? 0 : 1;
          return asort - bsort;
        });
        list.forEach((item: StaffType) => {
  ...
  const onInputChange = (e: any) => {
    ...
    if (val.trim()) {
      setFilterList(
        compassList.filter((item) => item.type === NodeTypeEnum.org && item.name.indexOf(val) > -1),
      );
    } else {
      setFilterList([]);
    }
  ...
  return (
    <Spin spinning={loading}>
      <div className="flowOrg">
        <div className="flowOrg-left">
          <div className="flowOrg-left-input">
            <Input
              placeholder="请输入组织名称"
              bordered={false}
              value={filterValue}
          ...
          {filterValue.trim() ? (
            ...
            <>
              <div className="flowOrg-left-bread">
                <Breadcrumb separator=">">
                  {breadList.map((item) => (
                    <Breadcrumb.Item key={item.code}>
                      <span className="flowOrg-left-bread-item" onClick={() => changeBread(item)}>
                        {item.name}
                      </span>
                    </Breadcrumb.Item>
                  ))}
              ...
              {currentBread?.children &&
                ...
                currentBread.children.every((item) => item.type === NodeTypeEnum.org) && (
        ...
        <div className="flowOrg-right">
          <div className="flowOrg-right-top">已选：{rightList.length}个组织</div>
          <div className="scroll-container">
            <div className="scroll-box">
              <div className="list">
                {rightList.map((item) => (
