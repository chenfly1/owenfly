...
export default function ElementBaseInfo(props: IProps) {
  const { handleProcessId, handleProcessName } = useModel('useFlow');
  // props
  ...
  const getDefaultName = () => {
    if (businessObject?.$type === 'bpmn:StartEvent') {
      return '开始节点';
    } else if (businessObject?.$type === 'bpmn:EndEvent') {
      return '结束节点';
    } else {
      return '';
    }
  ...
  function initPageData() {
    ...
    if (!name) {
      name = getDefaultName();
      updateElementAttr(keyOptions.name, name);
    }
    form.setFieldsValue({
  ...
  function updateElementAttr(key: string, value: any) {
    if (key === keyOptions.id) {
      ...
      if (!validateFlag) {
        ...
        try {
          ...
          message.error('编号已存在,当前修改未生效').then(() => console.log(e.message));
          return;
        }
      }
      // 更新id
    ...
    if (businessObject.$type === 'bpmn:Process') {
      if (key === keyOptions.id) {
        handleProcessId(value);
      } else if (key === keyOptions.name) {
        handleProcessName(value);
      }
    }
  }
        ...
        <>
          <Form.Item label="版本标签" name="versionTag">
            <Input
              placeholder={'请输入'}
              onChange={(event) => {
                updateElementAttr(keyOptions.versionTag, event.currentTarget.value);
              }}
            />
          </Form.Item>
