...
import { loop_characteristics_type, loop_characteristics_type_options } from './dataSelf';
...
export default function MultiInstance(props: IProps) {
  ...
  // 字段监听
  const loopCharacteristics = Form.useWatch('loopCharacteristics', multiInstanceForm);
  const asyncBefore = Form.useWatch('asyncBefore', asyncStatusForm);
  const asyncAfter = Form.useWatch('asyncAfter', asyncStatusForm);
  // redux
  const bpmnPrefix = useModel('useFlow', (model) => model.flowState.prefix);
  ...
  function updateLoopCharacteristics(value: string) {
    ...
    if (asyncBefore || asyncAfter) timeCycleForm.resetFields();
    // 取消多实例配置
    if (value === '-1') {
  ...
  function updateLoopCardinality(value: string) {
    ...
    if (value && value.length) {
      loopCardinality = window.bpmnInstance.moddle.create('bpmn:FormalExpression', { body: value });
    }
  ...
  function updateLoopTimeCycle(value: string) {
    const extensionElements = window.bpmnInstance.moddle.create('bpmn:ExtensionElements', {
      values: [
        window.bpmnInstance.moddle.create(`${bpmnPrefix}:FailedJobRetryTimeCycle`, {
          body: value,
        }),
      ],
    });
  ...
  function updateLoopBase() {
    window.bpmnInstance.modeling.updateModdleProperties(
      ...
      window.bpmnInstance.element.businessObject.loopCharacteristics,
      {
        ...
        elementVariable: multiInstanceForm.getFieldValue(keyOptions.elementVariable),
      },
    );
  }
  ...
  function updateLoopAsync() {
    let asyncAttr: any;
    const asyncBefore: boolean = asyncStatusForm.getFieldValue('asyncBefore');
    const asyncAfter: boolean = asyncStatusForm.getFieldValue('asyncAfter');
    const exclusive: boolean = asyncStatusForm.getFieldValue('exclusive');
  ...
  function renderTimeCycleForm() {
    return (
      <>
        <Form form={timeCycleForm} labelCol={{ span: 5 }} wrapperCol={{ span: 18 }}>
  ...
  return (
    <>
      <Form form={multiInstanceForm} labelCol={{ span: 5 }} wrapperCol={{ span: 18 }}>
        ...
        {(loopCharacteristics === loop_characteristics_type.parallelMultiInstance ||
          loopCharacteristics === loop_characteristics_type.sequentialMultiInstance) &&
