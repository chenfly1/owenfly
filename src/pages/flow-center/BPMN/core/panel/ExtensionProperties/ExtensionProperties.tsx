...
export default function ExtensionProperties(props: IProps) {
  ...
  function createOrUpdate(options: any) {
    ...
    // 创建属性实例
    const property: any = createProperty(prefix, {
      name: name,
      value: value,
    });
    // 创建扩展属性列表实例
    const newProperties: any[] = [...propertyList];
    newProperties.splice(index > 0 ? index - 1 : propertyList.length, 1, property);
    const properties: any = createProperties(prefix, {
