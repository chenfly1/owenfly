...
import { Button, Empty, Form, Input, notification, Select, Space, Table, Typography } from 'antd';
import { AlignRightOutlined, PlusOutlined } from '@ant-design/icons';
import EditFormField from './EditFormField/EditFormField';
import { checkIsCustomType, getFormFieldNameByType } from './dataSelf';
import { extractFormData } from '../../util/panelUtil';
...
export default function ElementForm(props: IProps) {
  ...
  const [businessKeyOptions, setBusinessKeyOptions] = useState<any[]>([]);
  // ref
  ...
  // model
  const { prefix: bpmnPrefix } = useModel('useFlow', (model) => ({
  ...
  function initPageData() {
    let businessObject: any = window.bpmnInstance?.element?.businessObject || props.businessObject;
    // 获取FormData
    ...
    // 获取表单字段
    let fields: any[] = encapsulateFormFields(formData);
    ...
    function encapsulateFormFields(formData: any) {
      // 获取表单字段
      let fields: any[] = JSON.parse(JSON.stringify(formData?.fields || []));
      fields = fields?.map((e, i) => {
        // 获取枚举值列表
        let enumValues: any[] = e.values;
        ...
        // 获取字段属性
        let properties: any[] = e.properties?.values;
        ...
        // 获取字段约束
        let constraints: any[] = e.validation?.constraints;
  ...
  function updateBusinessKey(option: any) {
    ...
    window.bpmnInstance.modeling.updateModdleProperties(window.bpmnInstance.element, formData, {
      businessKey: value,
    });
  }
   ...
   * @param fields
   */
  function createBusinessKeySelectOptions(fields: any[]) {
    let businessKeyOptions: any[] =
      fields?.map((e) => {
        return {
          name: e.label,
          value: e.id,
  ...
  function createOrUpdateFormFields(options: any) {
    ...
    // 更新表格
    let newFormFields: any[] = [...formFields];
    ...
    function createFieldConfigObj() {
      ...
      if (properties && properties.length) {
        const propertiesConfig: any[] = properties.map((e: any, i: number) => {
          return window.bpmnInstance.moddle.create(`${bpmnPrefix}:Property`, {
            id: e.id,
            value: e.value,
          });
        });
        field.properties = window.bpmnInstance.moddle.create(`${bpmnPrefix}:Properties`, {
          values: propertiesConfig,
        });
      }
      // 设置约束
      if (constraints && constraints.length) {
        const validationConfig: any[] = constraints.map((e: any, i: number) => {
          return window.bpmnInstance.moddle.create(`${bpmnPrefix}:Constraint`, {
            name: e.name,
            config: e.config,
          });
        });
        field.validation = window.bpmnInstance.moddle.create(`${bpmnPrefix}:Validation`, {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'type',
      ellipsis: true,
      render: (text: any) => (checkIsCustomType(text) ? text : getFormFieldNameByType(text)?.name),
    },
    {
      title: '默认值',
      width: 80,
    ...
    },
    {
      ...
      render: (text: string, record: any) => (
        <Space size="small">
          ...
          <Button danger type="text" size={'small'} onClick={() => removeFormField(record.key)}>
            {'删除'}
          </Button>
        </Space>
      ),
    ...
    <>
      <Form form={form} labelCol={{ span: 5 }} wrapperCol={{ span: 18 }}>
        ...
        <Form.Item name="businessKey" label="业务标识">
          <Select placeholder={'请选择'} onChange={(value, option) => updateBusinessKey(option)}>
      ...
      <EditFormField onRef={editFormFieldRef} reFreshParent={createOrUpdateFormFields} />
    </>
  );
}
