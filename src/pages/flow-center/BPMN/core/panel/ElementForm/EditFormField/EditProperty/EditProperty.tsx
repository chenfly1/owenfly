...
export default function EditProperty(props: IProps) {
  ...
  const { colorPrimary } = useModel('useFlow', (model) => ({
    colorPrimary: model.flowTheme.colorPrimary,
  }));
  // form
  ...
  function handleOK() {
    form
      .validateFields()
      .then((values) => {
        const rowObj: any = Object.create(null);
  ...
  return (
    <>
      <Modal
        ...
        title={<Typography style={{ color: colorPrimary }}>{'编辑属性'}</Typography>}
        open={open}
        okText={'确认'}
        cancelText={'取消'}
        onOk={handleOK}
      ...
      >
        <Form form={form} labelCol={{ span: 5 }} wrapperCol={{ span: 18 }}>
          ...
          <Form.Item label="值" name="value" rules={[{ required: true, message: '值不能为空哦!' }]}>
            <Input placeholder={'请输入'} />
          </Form.Item>
        </Form>
      </Modal>
