...
import { form_field_type, form_field_type_options, getFormFieldNameByType } from '../dataSelf';
import EditConstraint from './EditConstraint/EditConstraint';
import EditProperty from './EditProperty/EditProperty';
import EditEnumValues from './EditEnumValues/EditEnumValues';
...
export default function EditFormField(props: IProps) {
  ...
  const [enumValueRows, setEnumValueRows] = useState<any[]>([]);
  const [constraintRows, setConstraintRows] = useState<any[]>([]);
  ...
  function renderEnumValuesTable() {
    ...
    function createOrUpdateEnumValues(rowObj: any) {
      const { key } = rowObj;
      const newRows: any[] = [...enumValueRows];
      ...
      },
      {
        ...
        render: (text: string, record: any) => (
          <Space size={0}>
            ...
            <Button danger type="text" size={'small'} onClick={() => removeEnumValues(record.key)}>
              {'删除'}
            </Button>
          </Space>
        ),
    ...
    return (
      <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
        ...
        />
        {/* 编辑枚举列表组件*/}
        <EditEnumValues onRef={editEnumValuesRef} reFreshParent={createOrUpdateEnumValues} />
      </Space>
    );
  }
  ...
  function renderConstraintTable() {
    ...
    function createOrUpdateConstraint(rowObj: any) {
      const { key } = rowObj;
      const newRows: any[] = [...constraintRows];
    ...
    function removeConstraint(key: number) {
      let newRows: any[] = [...constraintRows];
      newRows.splice(key - 1, 1);
      newRows = newRows.map((el, index) => {
        if (el.key !== key) {
          el.key = index + 1;
      ...
      },
      {
        ...
        render: (text: string, record: any) => (
          <Space size={0}>
            ...
            <Button danger type="text" size={'small'} onClick={() => removeConstraint(record.key)}>
              {'删除'}
            </Button>
          </Space>
        ),
    ...
    return (
      <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
        ...
        />
        {/* 编辑约束条件 */}
        <EditConstraint onRef={editConstraintRef} reFreshParent={createOrUpdateConstraint} />
      </Space>
    );
  }
  ...
  function renderPropertyTable() {
    ...
    function createOrUpdateProperty(rowObj: any) {
      const { key } = rowObj;
      const newRows: any[] = [...propertyRows];
      ...
      },
      {
        ...
        render: (text: string, record: any) => (
          <Space size={0}>
            ...
            <Button danger type="text" size={'small'} onClick={() => removeProperty(record.key)}>
              {'删除'}
            </Button>
          </Space>
        ),
    ...
    return (
      <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
        ...
        />
        {/* 编辑字段属性组件 */}
        <EditProperty onRef={editPropertyRef} reFreshParent={createOrUpdateProperty} />
      </Space>
    );
  }
      ...
      >
        <Form labelCol={{ span: 5 }} wrapperCol={{ span: 18 }} form={form}>
          ...
          {type === form_field_type.custom && (
            <Form.Item label="类型名称" name="customType" rules={[{ required: true }]}>
              <Input placeholder={'请输入'} />
            </Form.Item>
          )}
          {type === form_field_type.date && (
            <Form.Item label="日期格式" name="datePattern" rules={[{ required: true }]}>
              <Input placeholder={'请输入'} />
            </Form.Item>
          )}
