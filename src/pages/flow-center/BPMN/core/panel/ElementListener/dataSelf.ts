...
function findEventType(listener: any) {
  let eventType: any = execute_event_type_options.find((e) => e.value === listener.event);
  if (!eventType) {
...
function findListenerType(listener: any) {
  ...
  if (listener.class) {
    listenerType = listener_type_options.find((e) => e.value === listener_type.class);
  } else if (listener.expression) {
    listenerType = listener_type_options.find((e) => e.value === listener_type.expression);
  } else if (listener.delegateExpression) {
    listenerType = listener_type_options.find((e) => e.value === listener_type.delegateExpression);
  } else if (listener.script) {
    listenerType = listener_type_options.find((e) => e.value === listener_type.script);
  }
  return listenerType;
}
...
function findScriptType(listener: any) {
  ...
  if (listener.script?.value) {
    scriptType = script_type_options.find((e) => e.value === script_type.inlineScript);
  } else if (listener.script?.resource) {
    scriptType = script_type_options.find((e) => e.value === script_type.externalResource);
  }
  return scriptType;
}
...
export function encapsulateListener(listener: any) {
  const listenerType: any = findListenerType(listener);
  const eventType: any = findEventType(listener);
  const scriptType: any = findScriptType(listener);
  return {
...
export function encapsulateField(field: any) {
  ...
  if (field.string) {
    ...
    fieldType = listener_type_options.find((e) => e.value === field_type.expression);
  } else {
