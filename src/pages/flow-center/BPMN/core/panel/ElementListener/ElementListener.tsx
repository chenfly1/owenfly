...
import { encapsulateListener } from './dataSelf';
import EditListener from './EditListener/EditListener';
import {
...
export default function ElementListener(props: IProps) {
  ...
  // redux
  const prefix = useModel('useFlow', (model) => model.flowState.prefix);
  ...
  function initRows() {
    // 获取监听器
    const listeners: any[] = extractExtensionList(
      prefix,
      `${isTask ? ELEMENT_LISTENER_TYPE.TaskListener : ELEMENT_LISTENER_TYPE.ExecutionListener}`,
    );
    setListenerList(listeners);
    // 设置行数据源
    const rows: any[] = listeners?.map((e, i) => {
      const listener = encapsulateListener(e);
      return {
        key: i + 1,
  ...
  function getOtherExtensionList() {
    const otherExtensionList: any[] = extractOtherExtensionList(
      prefix,
      `${isTask ? ELEMENT_LISTENER_TYPE.TaskListener : ELEMENT_LISTENER_TYPE.ExecutionListener}`,
    );
  ...
  function createOrUpdate(options: any) {
    // 创建监听器对象
    const listener: any = Object.create(null);
    ...
    // 创建监听器实例
    const listenerObject = createListenerObject(listener, isTask, prefix);
    // 将监听器实例绑定到bpmn
    const newListenerExtensionList: any[] = [...listenerList];
    newListenerExtensionList.splice(
      options.rowKey > 0 ? options.rowKey - 1 : listenerList.length,
      1,
      listenerObject,
    );
    updateElementExtensions(getOtherExtensionList().concat(newListenerExtensionList));
    // 刷新表格
    initRows();
  }
        ...
        },
        {
          ...
          render: (text: string, record: any) => (
            <Space size="small">
              ...
              <Button danger type="text" size={'small'} onClick={() => remove(record.key)}>
                {'删除'}
              </Button>
            </Space>
          ),
  ...
  return (
    <>
      ...
      {/* 弹窗组件 */}
      <EditListener onRef={editRef} isTask={isTask} reFreshParent={createOrUpdate} />
    </>
  );
}
