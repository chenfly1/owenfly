...
export default function EditListener(props: IProps) {
  ...
  function initPageData(rowObj: any) {
    ...
    // 初始化rows
    const fields: any[] = rowObj?.protoListener?.fields || [];
    const rows: any[] = fields.map((el, index) => {
      const field: any = encapsulateField(el);
    ...
    },
    {
      ...
      render: (text: string, record: any) => (
        <Space size={0}>
          ...
          <Button danger type="text" size={'small'} onClick={() => removeField(record.key)}>
            {'删除'}
          </Button>
        </Space>
      ),
        ...
        <Form labelCol={{ span: 5 }} wrapperCol={{ span: 18 }} form={form}>
          ...
          >
            <Select placeholder={'请选择'}>
              {(isTask ? task_event_type_options : execute_event_type_options).map((e) => {
                return (
                  <Option key={e.value} value={e.value}>
                    {e.name}
                  </Option>
