...
import ScriptTask from './ScriptTask/ScriptTask';
...
export default function ElementTask(props: IProps) {
  ...
  function updateTaskAsync() {
    const asyncBefore: boolean = form.getFieldValue('asyncBefore');
    const asyncAfter: boolean = form.getFieldValue('asyncAfter');
  ...
  return (
    <>
      ...
      {businessObject?.$type === 'bpmn:UserTask' && <UserTask businessObject={businessObject} />}
