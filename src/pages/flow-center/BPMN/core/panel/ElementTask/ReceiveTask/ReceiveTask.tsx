...
export default function ReceiveTask(props: IProps) {
  ...
  const [messageRefOptions, setMessageRefOptions] = useState<any[]>([defaultOptions]);
  // redux
  ...
  function initPageData() {
    // 初始化下拉项
    const rootElements: any[] = window.bpmnInstance?.modeler.getDefinitions().rootElements || [];
    const options: any[] = rootElements
    ...
    // 初始化form初始值
    form.setFieldsValue({
      messageId: window.bpmnInstance?.element.businessObject?.messageRef?.id || '-1',
      id: undefined,
      name: undefined,
    });
  }
  ...
  function handleOK() {
    form
      ...
      .then((values) => {
        // 新增消息实例并设置消息实例
        const messageConfig: any = window.bpmnInstance.moddle.create('bpmn:Message', {
          id: values.id,
          name: values.name,
        });
        window.bpmnInstance?.modeler.getDefinitions().rootElements.push(messageConfig);
    ...
    return (
      <>
        ...
        <Modal
          ...
          title={<Typography style={{ color: colorPrimary }}>{'创建消息实例'}</Typography>}
          open={open}
          okText={'确认'}
          cancelText={'取消'}
          onOk={handleOK}
          ...
          <Select
            ...
            dropdownRender={(menu) => (
              <>
                ...
                <Space align="end" direction={'vertical'} style={{ padding: '0 8px 4px' }}>
                  <CreateNewMessage />
                </Space>
              </>
            )}
