...
import { assignee_mock, candidateGroups_mock, candidateUsers_mock } from '../mockData';
...
export default function UserTask(props: IProps) {
  ...
  function initPageData() {
    form.setFieldsValue({
      ...
      candidateUsers: businessObject?.candidateUsers ? [businessObject.candidateUsers] : [],
      candidateGroups: businessObject?.candidateGroups ? [businessObject.candidateGroups] : [],
      dueDate: businessObject?.dueDate && moment(businessObject.dueDate, true),
      followUpDate: businessObject?.followUpDate && moment(businessObject.followUpDate, true),
  ...
  function updateUserTask(key: string) {
    ...
    if (key === keyOptions.candidateUsers || key === keyOptions.candidateGroups) {
    ...
    window.bpmnInstance.modeling.updateProperties(window.bpmnInstance.element, taskAttr);
  }
    ...
    <>
      <Form form={form} labelCol={{ span: 5 }} wrapperCol={{ span: 18 }}>
        <Form.Item name="assignee" label="处理用户">
          <Select placeholder={'请选择'} onChange={() => updateUserTask(keyOptions.assignee)}>
            {assignee_mock.map((e) => {
              return (
