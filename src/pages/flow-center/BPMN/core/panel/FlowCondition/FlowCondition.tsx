...
export default function FlowCondition(props: IProps) {
  ...
  // watch
  const flowType = useWatch('flowType', form);
  const conditionType = useWatch('conditionType', form);
  const scriptType = useWatch('scriptType', form);
  ...
  function updateFlowType(value: string) {
    ...
    if (value === flow_type.conditionalFlow) {
      const flowConditionRef = window.bpmnInstance.moddle.create('bpmn:FormalExpression');
      window.bpmnInstance.modeling.updateProperties(element, {
