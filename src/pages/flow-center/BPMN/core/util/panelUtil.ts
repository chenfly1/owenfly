...
import {
  ...
  script_type,
  task_event_type,
} from '../panel/ElementListener/dataSelf';
import { UUIdGenerator } from './idUtil';
...
export function extractExtensionList(prefix: string, suffix: string) {
  const extensionElements: any = window.bpmnInstance?.element.businessObject.extensionElements;
  const list: any[] =
    extensionElements?.values?.filter((e: any) => e.$type === `${prefix}:${suffix}`) || [];
  return list;
}
...
export function extractOtherExtensionList(prefix: string, suffix: string) {
  const extensionElements: any = window.bpmnInstance?.element.businessObject.extensionElements;
  const list: any[] =
    extensionElements?.values?.filter((el: any) => el.$type !== `${prefix}:${suffix}`) || [];
  return list;
}
 ...
 * @param extensionList
 */
export function updateElementExtensions(extensionList: any[]) {
  const extensions = window.bpmnInstance?.moddle?.create('bpmn:ExtensionElements', {
...
export function extractPropertiesExtension(prefix: string) {
  ...
  const list: any[] =
    extensionElements?.values?.find((e: any) => e.$type === `${prefix}:Properties`)?.values || [];
  return list;
}
...
export function createFieldObject(option: any, prefix: string) {
  ...
  const fieldConfig = fieldType === field_type.string ? { name, string } : { name, expression };
...
export function createScriptObject(options: any, prefix: string) {
  ...
  const scriptConfig =
    scriptType === script_type.inlineScript ? { scriptFormat, value } : { scriptFormat, resource };
 ...
 * @param prefix
 */
export function createListenerObject(options: any, isTask: boolean, prefix: string) {
  ...
  // 任务监听器的 定时器 设置
  if (isTask && options.event === task_event_type.timeout && !!options.timerType) {
...
export function extractFormData(prefix: string) {
  ...
  // 获取或新建FormData
  const formData: any =
    extensionElements?.values?.filter((e: any) => e.$type === `${prefix}:FormData`)?.[0] ||
