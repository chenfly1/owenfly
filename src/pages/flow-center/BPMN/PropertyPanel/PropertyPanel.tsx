...
import ElementBaseInfo from '../core/panel/ElementBaseInfo/ElementBaseInfo';

import ElementDocument from '../core/panel/ElementDocument/ElementDocument';
import ExtensionProperties from '../core/panel/ExtensionProperties/ExtensionProperties';
import SignalMessage from '../core/panel/SignalMessage/SignalMessage';
import ElementListener from '../core/panel/ElementListener/ElementListener';
import ElementTask from '../core/panel/ElementTask/ElementTask';
import MultiInstance from '../core/panel/MultiInstance/MultiInstance';
import ElementForm from '../core/panel/ElementForm/ElementForm';
import {
  BellOutlined,
...
import { initBpmnInstance } from '../core/util/windowUtil';
import FlowCondition from '../core/panel/FlowCondition/FlowCondition';
import { useModel } from 'umi';
import ElementConfigure from '../core/panel/ElementConfigure';
import ElementCondition from '../core/panel/ElementCondition';
...
export default function PropertyPanel(props: IProps) {
  ...
  // redux
  const processId = useModel('useFlow', (model) => model.flowState.processId);
  const colorPrimary = useModel('useFlow', (model) => model.flowTheme.colorPrimary);
  ...
  useEffect(() => {
    initBpmnInstance();
    // 避免初始化，流程图未加载完导致出错
    console.log('property_1', modeler);
    if (modeler) {
      init();
    }
  }, [processId]);
  ...
  useEffect(() => {
    ...
    if (modeler) {
      ...
      modeler?.on('element.changed', ({ element }: any) => {
        if (element && element.id === window.bpmnInstance.element.businessObject.id) {
  ...
  function confirmCurrentElement(element: any) {
    ...
    if (!element) {
      ...
      const processElement: any = modeler.get('elementRegistry').get(newId);
      setElement(processElement);
      window.bpmnInstance.element = processElement;
      setBusinessObject(JSON.parse(JSON.stringify(processElement?.businessObject || null)));
      return;
    }
  ...
  function renderElementBaseInfo() {
    return (
      <Collapse.Panel
        header={
          <Typography style={{ color: colorPrimary, fontWeight: 'bold' }}>常规信息</Typography>
        }
        key="base"
        showArrow={true}
        forceRender={false}
   ...
   * 1、任务节点和发起节点都有
   */
  function renderElementConfigure() {
    if (element?.type.indexOf('Task') !== -1 || element?.type === 'bpmn:StartEvent') {
      return (
        <Collapse.Panel
          header={
            <Typography style={{ color: colorPrimary, fontWeight: 'bold' }}>任务信息</Typography>
          }
          key="configure"
          showArrow={true}
          forceRender={false}
        >
          <ElementConfigure businessObject={businessObject} />
        </Collapse.Panel>
      );
    }
  }
  ...
  function renderFlowCondition() {
    const conditionFormVisible: boolean = !!(
    ...
    if (conditionFormVisible) {
      return (
        <Collapse.Panel
          header={
            <Typography style={{ color: colorPrimary, fontWeight: 'bold' }}>流转条件</Typography>
          }
          key="condition"
          showArrow={true}
          forceRender={false}
        >
          ...
          <ElementCondition businessObject={businessObject} />
        </Collapse.Panel>
      );
    }
  }
    ...
    <>
      <Space direction="vertical" size={0} style={{ display: 'flex' }}>
        <Collapse
          ...
          accordion={false}
          defaultActiveKey={['base', 'configure', 'condition']}
          destroyInactivePanel={true}
        >
          {renderElementBaseInfo()}
          {renderElementConfigure()}
          {renderFlowCondition()}
          {/* {renderSignalMessage()}
          {renderElementForm()}
          {renderElementTask()}
