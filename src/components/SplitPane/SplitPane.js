...
class SplitPane extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      closeList: this.getPanePropSize(props).map((_) => {
        return false;
      }),
    };
  }
  ...
  onClosePane = (event, index) => {
    const closeList = this.state.closeList;
  ...
  render() {
    ...
    const elements = notNullChildren.reduce((acc, child, idx) => {
      ...
      const paneProps = {
        ...
        resizersSize,
        size: sizes[idx],
        closed: this.state.closeList[idx],
      };
      ...
      if (acc.length === 0) {
        ...
        const resizer = (
          <Resizer
            ...
            split={split}
            closed={this.state.closeList[resizerIndex]}
            onMouseDown={this.onMouseDown}
            onTouchStart={this.onTouchStart}
            onClosePane={this.onClosePane}
            onOpenPane={this.onOpenPane}
          />
        );
    ...
    return (
      <StyleComponent
        ...
        data-type="SplitPane"
        style={{
          borderTop: split === 'vertical' ? '1px solid rgba(5, 5, 5, 0.06)' : 0,
          marginTop: split === 'vertical' ? '16px' : '8px',
        }}
        data-split={split}
        innerRef={(el) => {
          this.splitPane = el;
